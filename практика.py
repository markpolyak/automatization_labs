...
#------ проверка на выполнение лабы по ссылке. Если лаба принята возвращвет дату если нет то None, если ссылка не верная то "#404" 
def check_lab(url_lab):
# чтение страницы
    s=requests.get(url_lab+'/commits/master')
# если нет такой страницы
    if s.ok is False:
        return '#404'
# разбор страницы
    b=bs4.BeautifulSoup(s.text, "html.parser")
# выбор всех объектов содежащих класс table-list-cell
    s_commits=b.select('.table-list-cell')
    for s_commit in s_commits:
        l_commit=s_commit.select('.text-green')
#        print(l_commit, len(l_commit))
        if len(l_commit)>0:
            date_commit=s_commit.select('relative-time')
#            print('ok', date_commit[0].text)
#            print(date_commit[0].attrs, date_commit[0].attrs.items())
            return date_commit[0].text
    return None
...
for current_index_email in list_email:
    ...
    if 'https://github.com/k43guap/os' == list_repozit[0]:
        ...
        if list_repozit[1] in list_lab:
            ...
            data_comite=check_lab(repozit) # проверка даты выполнения лабы 
            if data_comite is not None:
                print(data_comite)
                worksheet.update_cell(number_row, number_col+1, data_comite) # запись даты выполнения лабы или 404 если ссылка не правильная
